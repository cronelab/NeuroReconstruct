...
| `services/structure_extractor.py` | **ACTIVE — patient-specific, wired to UI.** antspynet `deep_atropos` (subcortical) + `desikan_killiany_tourville_labeling` (cortical DKT) run on the patient's own T1 — native space, no MNI atlas. Extracts ~84 structures across 6 groups as meshes aligned to the brain mesh. Cached as per-structure JSON + `structures_cortical.nii.gz`. (Superseded the old parked nilearn/Harvard-Oxford MNI152 approach.) **Mesh smoothing:** marching cubes at the boundary-preserving isolevel 0.5 (the old 0.4 inflated every structure ~+4.9%), Gaussian sigma scaled to each structure's half-thickness (0.5 for thin ribbons → 0.8 for bulky ones, since a flat sigma erodes thin parcels ~10%), then shrink-free Taubin smoothing. Measured on PY26N009_dev3: mean absolute volume error vs the label mask 4.9% → **2.7%**, 76/78 structures more accurate, 78/78 smoother. **Display meshes only — the label volume is untouched, so `contact_labeling.py` output is unaffected.** |
| `services/contact_labeling.py` | **Export pipeline.** Labels each contact with the patient-specific brain structure it sits in, from the cached DKT volume `structures_cortical.nii.gz` (native space — no atlas). Depth contacts often sit in white matter (~43% direct hits on real data), so when the contact voxel is unlabeled it runs a **plurality vote** over all catalog-labeled voxels within **2 mm** — the structure occupying most of the neighbourhood wins (ties break toward the closest voxel), rather than letting a single stray voxel decide. `vote_share` reports the winner's fraction (1.0 = unanimous / direct hit), `distance_mm=0` means the contact is inside. Lifts useful coverage from ~43% to ~83% on real data. The radius is deliberately tight — at 5 mm coverage reaches ~97%, but the extra assignments sit 2–5 mm from the named structure, which is weak evidence about where a contact really is; leaving those unassigned is the more honest default. Tunable via `label_contacts(search_radius_mm=...)`. Ventricle/CSF/WM voxels are named via `NON_CATALOG_LABELS` and surfaced in `voxel_content`. Writes `electrodes_structures.csv`. |
